...
const server = setupServer(
  // 共通で使用するMSWのハンドラーで10000円を返すように設定
...
describe('ページコンポーネント', () => {
  test('初期表示の確認', async () => {
    ...
    expect(screen.getByLabelText('退職起因')).not.toBeChecked();
  ...
  test('退職起因チェックボックスを選択できる', async () => {
    const pendingRequest = waitForCalcTaxRequest();

    const user = userEvent.setup();
    render(<Page />);

    await user.click(screen.getByLabelText('退職起因'));
  ...
  test('退職起因チェックボックスを未洗濯にできる', async () => {
    const pendingRequest = waitForCalcTaxRequest();
